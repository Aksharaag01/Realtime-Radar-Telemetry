...
void data_handler(z_loaned_sample_t* sample, void* arg) {
	...
	cout << "[RECEIVED on "
		<< string(z_string_data(z_loan(key_str)), z_string_len(z_loan(key_str)))
		<< "] -> "
...
int main(int argc, char* argv[]) {
	cout << "======================================================" << endl;
	cout << "          ZENOH TELEMETRY RECEIVER                    " << endl;
	cout << "======================================================" << endl;
	...
	cout << "Zenoh session connected. Listening for telemetry..." << endl;
	z_owned_closure_sample_t callback;
	z_closure_sample(&callback, data_handler, NULL, NULL);

	z_view_keyexpr_t keyexpr;
	z_view_keyexpr_from_str(&keyexpr, "simulation/aircraft/position");

	z_owned_subscriber_t sub;
	if (z_declare_subscriber(z_loan(session), &sub, z_loan(keyexpr), z_move(callback), NULL) < 0) {
		cerr << "Failed to declare Zenoh subscriber!" << endl;
